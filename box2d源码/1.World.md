...
创建世界 的时候将数据设置，我们一般会传入一个重力的向量，我们将数据传递给指定的变量就可以了，其他的数据是默认值
...
b2ContactFilter defaultFilter;
	 ...
	 Java_com_badlogic_gdx_physics_box2d_World_newWorld(JNIEnv* env,
	  jobject object, jfloat gravityX, jfloat gravityY, jboolean doSleep) {
		if(!worldClass) {
...
世界在模拟的过程中进行 模拟，处于锁定状态，这个时候是不可以进行创建操作的。创建body需要分配空间，然后将其加入到双向链表里面。
...
如果这是二者是一样的，那么就不需要进行从新设置 ，如果不一样，遍历所有的body然后进行一次重新设置。
